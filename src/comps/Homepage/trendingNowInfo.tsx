    ...
    return (
        <div className='min-h-fit w-full flex flex-col justify-start items-start gap-2 max-h-fit pt-4 px-4'>
            {trending.albums.items.map((item) => {
                return (
                    <Link key={item.id} to={`search/${item.id}`} className='flex w-full items-center gap-2 text-start hover:bg-black hover:bg-opacity-30 duration-200 p-3 rounded-lg max-h-fit'>
                        <img src={item.images[0].url} className='rounded-full xl:w-1/12 w-1/5' />
